...
- (void)viewDidLoad {
    ...
    [self contrastNews];
    [self sendRequest:@"HistoryNews" andPath:queryURL andSqlParameter:@[CHATCODE,self.chatItem.from,@"群聊",[self getTomorrowTime]] and:self];
...
- (void)contrastNews{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([[ArchiveClass new] contrastNews:self.chatItem.from andChatType:@"home"]) {
                [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:@"OfflineHaveNews" object:[[ArchiveClass new] getLocalNews]]];
            }
        });
    });
}
...
- (void)creatUI{
    ...
    [chatView creatUI:self.chatItem.from andName:self.chatItem.fromName andFace:self.chatItem.fromFace andType:@"home" andOwer:@""];
...
- (void)getHistoryNews{
    ...
    [self sendRequest:@"HistoryNews" andPath:queryURL andSqlParameter:@[CHATCODE,self.chatItem.from,@"群聊",self.getHistoryString] and:self];
}
...
-(void)saveMessage:(ChatItem *)chatItem{
    NSArray *sqlParameter=@[chatItem.from,chatItem.fromName,chatItem.to,chatItem.toName,chatItem.from,@"",chatItem.content,chatItem.contentType,@"群聊",@"iOS",chatItem.fromFace,chatItem.toFace];
