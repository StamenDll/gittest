...
@interface CRNewsViewController ()

@end

@implementation CRNewsViewController
...
- (void)viewDidLoad {
    ...
    [self sendRequest:@"HistoryNews" andPath:queryURL andSqlParameter:@[CHATCODE,self.chatRoomItem.LID,@"公众",[self getTomorrowTime]] and:self];
    [self sendRequest:@"ChatRoomPeople" andPath:queryURL andSqlParameter:@[self.chatRoomItem.LID,CHATCODE,@"1"] and:self];
    [self sendRequest:@"OnOrOffLine" andPath:queryWithoutURL andSqlParameter:@[self.chatRoomItem.LID,CHATCODE,@"在线"] and:self];
}

- (void)popViewController{
    [self.navigationController popViewControllerAnimated:YES];
    [self sendRequest:@"OnOrOffLine" andPath:queryWithoutURL andSqlParameter:@[self.chatRoomItem.LID,CHATCODE,@"离开"] and:self];
}
- (void)addRightButtonItem{
    UIButton *rButton=[UIButton buttonWithType:UIButtonTypeCustom];
    rButton.frame=CGRectMake(0, 0, 30,30);
    [rButton setImage:[UIImage imageNamed:@"group"] forState:UIControlStateNormal];
    rButton.imageEdgeInsets=UIEdgeInsetsMake(5,7,5,0);
    [rButton addTarget:self action:@selector(lookCRMessage) forControlEvents:UIControlEventTouchUpInside];
...
- (void)lookCRMessage{
    CRMessageViewController *cvc=[[CRMessageViewController alloc]init];
    cvc.chatRoomItem=self.chatRoomItem;
    [self.navigationController pushViewController:cvc animated:YES];
}

- (void)creatUI{
    chatView=[[ChatView alloc]initWithFrame:CGRectMake(0,64, mainWidth, mainHeight-64)];
    [chatView creatUI:self.chatRoomItem.LID andName:self.chatRoomItem.LRoomName andFace:self.chatRoomItem.LPic andType:@"group" andOwer:self.chatRoomItem.LMainDoctorCode];
    chatView.delegate=self;
    [self.view addSubview:chatView];
    
    if (self.chatRoomItem.LHeadUrl.length>0) {
        UIButton *noticeButton=[self addButton:CGRectMake(20, 64, SCREENWIDTH-40, 75) adnColor:[self colorWithHexString:@"e5e5e5"] andTag:0 andSEL:@selector(readNotice:)];
        [self.view addSubview:noticeButton];
        
        UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:noticeButton.bounds byRoundingCorners:UIRectCornerBottomLeft | UIRectCornerBottomRight cornerRadii:CGSizeMake(5, 5)];
        CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
        maskLayer.frame = noticeButton.bounds;
        maskLayer.path = maskPath.CGPath;
        noticeButton.layer.mask = maskLayer;
        
        UIButton *delButton=[self addSimpleButton:CGRectMake(noticeButton.frame.size.width-30,10,20,20) andBColor:MAINWHITECOLOR andTag:0 andSEL:@selector(deleteNotice:) andText:@"X" andFont:MIDDLEFONT andColor:GREENCOLOR andAlignment:1];
        [delButton.layer setCornerRadius:10];
        [noticeButton addSubview:delButton];
        
        
        UIImageView *noticeImageView=[[UIImageView alloc]initWithFrame:CGRectMake(10,10, 50, 50)];
        [noticeImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",PICURL,self.chatRoomItem.LHeadPic]] placeholderImage:[UIImage imageNamed:@"notice_default"]];
        [noticeImageView.layer setCornerRadius:5];
        noticeImageView.layer.masksToBounds=YES;
        [noticeButton addSubview:noticeImageView];
        
        UILabel *titleLabel=[self addLabel:CGRectMake(70,25, noticeButton.frame.size.width-80, 20) andText:self.chatRoomItem.LHeadTitle andFont:BIGFONT andColor:TEXTCOLOR andAlignment:0];
        [noticeButton addSubview:titleLabel];
    }
}

- (void)deleteNotice:(UIButton*)button{
    [UIView animateWithDuration:1 animations:^{
        button.superview.frame=CGRectMake(20, 0, SCREENWIDTH-40, 75);
        [button.superview removeFromSuperview];
    }];
}

- (void)readNotice:(UIButton*)button{
    [button removeFromSuperview];
    if ([self changeNullString:self.chatRoomItem.LHeadUrl].length>0) {
    CommenWebViewController *pvc=[CommenWebViewController new];
    if ([self.chatRoomItem.LHeadUrl hasPrefix:@"http"]) {
        pvc.mainUrl=self.chatRoomItem.LHeadUrl ;
    }else{
        pvc.mainUrl=[NSString stringWithFormat:@"%@%@",PICURL,self.chatRoomItem.LHeadUrl];
    }
    ...
    if ([message isKindOfClass:[NSArray class]]) {
        ...
        if ([type isEqualToString:@"HistoryNews"]) {
            ...
            if (data.count>0) {
                ...
                chatView.peopleItem=[RMMapper objectWithClass:[ChatRoomPeopleItem class] fromDictionary:dic];
                if ([[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]>0&&[[dic objectForKey:@"LostTime"] intValue]<[[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]) {
                    chatView.isCanSend=@"2";
                }else if ([[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]==0||[[dic objectForKey:@"LostTime"] intValue]>[[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]){
                    chatView.isCanSend=@"1";
                    if ([usd objectForKey:self.chatRoomItem.LID]) {
                        [usd removeObjectForKey:self.chatRoomItem.LID];
                    }
                }
            }
        }
        else if ([type isEqualToString:@"PeopleState"]){
            if (data.count>0) {
                NSDictionary *dic=[data objectAtIndex:0];
                chatView.peopleItem=[RMMapper objectWithClass:[ChatRoomPeopleItem class] fromDictionary:dic];
                if ([[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]>0&&[[dic objectForKey:@"LostTime"] intValue]<[[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]) {
                    chatView.isCanSend=@"2";
                    [self showSimplePromptBox:self andMesage:[NSString stringWithFormat:@"由于某些原因，您被管理员禁言%d分钟，请%d分钟后稍候再试！",[[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue],[[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]-[[dic objectForKey:@"LostTime"] intValue]]];
                }else if ([[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]==0||[[dic objectForKey:@"LostTime"] intValue]>[[dic objectForKey:@"LForbiddenTalkTimeLength"] intValue]){
                    chatView.isCanSend=@"1";
                    if ([usd objectForKey:self.chatRoomItem.LID]) {
                        [usd removeObjectForKey:self.chatRoomItem.LID];
...
- (void)getHistoryNews{
    ...
    [self sendRequest:@"HistoryNews" andPath:queryURL andSqlParameter:@[CHATCODE,self.chatRoomItem.LID,@"公众",self.getHistoryString] and:self];
}

-(void)saveMessage:(ChatItem *)chatItem{
    NSArray *sqlParameter=@[chatItem.from,chatItem.fromName,chatItem.to,chatItem.toName,chatItem.from,@"",chatItem.content,chatItem.contentType,@"公众",@"iOS",chatItem.fromFace,chatItem.toFace];
...
//查询状态
- (void)getUserState{
    [self sendRequest:@"PeopleState" andPath:queryURL andSqlParameter:@[self.chatRoomItem.LID,CHATCODE,@"1"] and:self];
}
