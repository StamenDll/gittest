...
- (void)viewDidLoad {
    ...
    [self creatUI];
    nowCount=1;
    [self sendRequest:@"QuestionList" andPath:queryURL andSqlParameter:@[@"",CHATCODE,@"我收藏",@"1"] and:self];
}
...
- (void)requestSuccess:(id)message andType:(NSString *)type{
    if ([message isKindOfClass:[NSArray class]]) {
        ...
        if ([type isEqualToString:@"QuestionList"]) {
            ...
            [mainTableView deleteSections:[NSIndexSet indexSetWithIndex:delIndex] withRowAnimation:UITableViewRowAnimationNone];
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"删除" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        [self showPromptBox:self andMesage:@"确定要删除该收藏信息吗？" andSel:@selector(sureDel)];
    }];
    return @[deleteAction];
}
...
- (void)sureDel{
    ...
    [self sendRequest:@"DelCollect" andPath:excuteURL andSqlParameter:@[fram.questionMessage.LID] and:self];
}
...
- (void)refresh{
    ...
    mainTableView.mj_header= [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        [self sendRequest:@"QuestionList" andPath:queryURL andSqlParameter:@[@"",CHATCODE,@"我收藏",[NSString stringWithFormat:@"%d",nowCount]] and:self];
    }];
